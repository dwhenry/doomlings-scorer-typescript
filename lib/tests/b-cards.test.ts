...
describe('Using BIONIC ARM', () => {
  it('when on a single card', () => {
    ...
    ).toMatchObject({ finalA: -1, finalB: 1 });
  });
  ...
  it('when multiple techlings', () => {
    ...
    expect(
      scores.getPlayerScore(Player.One).getCardScoreByIndex(0)
    ).toMatchObject({ finalA: -1, finalB: 3, total: 2 });
  });
  ...
  it('1 for each techling', () => {
    ...
    expect(
      scores.getPlayerScore(Player.One).getCardScoreByIndex(0)
    ).toMatchObject({ finalA: -1, finalB: 5, total: 4 });
  });
});
...
describe('Using BOREDOM + other cards', () => {
  it('add points when cards have effects', () => {
    ...
    expect(
      scores.getPlayerScore(Player.One).getCardScoreByIndex(0)
    ).toMatchObject({ finalA: 0, finalB: 2, total: 2 });
  });
  ...
  it('does not add points when cards have no effects', () => {
    ...
    expect(
      scores.getPlayerScore(Player.One).getCardScoreByIndex(0)
    ).toMatchObject({ finalA: 0, finalB: 1, total: 1 });
  });
});
