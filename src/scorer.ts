...
export class Scorer {
  ...
  constructor(catastopheInput: Array<PlayerInput>, ...cardsInput: Array<Array<PlayerInput>>) {
    ...
    this.catastopheCards = catastopheInput.map((playerInput: PlayerInput): CardInstance => getCard(playerInput.name, playerInput))
  }
  ...
  scores(): number[] {
    ...
    const result: number[] = this.allPlayerCards.map((playerCards) => {
      return playerCards.reduce((sum, inst) => sum + inst.finalA + inst.finalB || 0, 0)
    })
