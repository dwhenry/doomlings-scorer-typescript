...
const bionic_arm: Card = {
  ...
  type: 'red',
  pack: 'Techlings',
  calcA: (inst: CardInstance): void => { inst.finalA = -1 },
  calcB: (inst: CardInstance, allPlayerCards: Array<Array<CardInstance>>, currentPlayer: number): void => {
    let playerCards = allPlayerCards[currentPlayer];
    const techlingCards = playerCards.filter((inst) => inst.card.pack === 'Techlings')
